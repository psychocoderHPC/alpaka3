...
namespace simd_access
{
    ...
    template<class PotentialSimdType>
    concept any_simd = stdx_simd<PotentialSimdType> || requires(std::remove_cvref_t<PotentialSimdType> x) {
...
namespace simd_access
{
    ...
    struct linear_location
    {
        ...
        {
            return linear_location<std::remove_reference_t<decltype(std::declval<T>().*Member)>, SimdModel>{
                &(base_->*Member)};
        }
    ...
    struct indexed_location
    {
        ...
        auto member_access() const
        {
            return indexed_location<
                ...
                SimdModel,
                ArrayType>{&(base_->*Member), indices_};
        }
...
namespace simd_access
{
    ...
    struct index
    {
         ...
         */
        auto scalar_index(auto i) const
        {
            return index_ + IndexType(i);
        }
         ...
         * @return A value_access representing a simd access expression to a consecutive sequence of elements in an
         * array.
         */
        template<class T>
        auto operator[](T* data) const
        {
            return value_access<linear_location<T, SimdModel>, sizeof(T)>(
                linear_location<T, SimdModel>{data + index_});
        }
    ...
    concept simd_index = (stdx_simd<PotentialIndexType> && std::is_integral_v<typename PotentialIndexType::value_type>)
                         || requires(std::remove_cvref_t<PotentialIndexType> x) {
                                []<class SimdModel, class IndexType>(index<SimdModel, IndexType>&) {}(x);
                            };
    ...
    inline auto scalar_index(index<SimdModel, IndexType> const& idx, auto i)
    {
    ...
    inline auto scalar_index(stdx::simd<IndexType, SimdModel> const& idx, auto i)
    {
        return idx[i];
    }
     ...
     */
    inline constexpr auto is_simd_index(auto&& idx)
    {
...
namespace simd_access
{
    ...
    inline void simd_members(FN&& func, DestType& d, typename DestType::value_type const& s)
    {
        func(d, s);
    }

    template<class FN, stdx_simd SrcType>
    inline void simd_members(FN&& func, typename SrcType::value_type& d, SrcType const& s)
    {
        func(d, s);
    }
    ...
    inline auto simdized_value(std::vector<T> const& v)
    {
    ...
    inline auto simdized_value(std::pair<T, U> const& v)
    {
    ...
    inline auto load(linear_location<T, SimdModel> const& location)
    {
    ...
    /**
     * Creates a simd value from rvalues returned by the functor `subobject` applied to the range of elements defined
     * by the simd index `idx`.
    ...
    inline auto load_rvalue(auto&& base, IndexType const& idx, auto&& subobject)
    {
        decltype(simdized_value<index_model_t<IndexType>>(std::declval<BaseType>())) result;
        for(decltype(idx.size()) i = 0, e = idx.size(); i < e; ++i)
        {
            simd_members(
                [&](auto&& dest, auto&& src) { dest[i] = src; },
                result,
                subobject(base[scalar_index(idx, i)]));
        }
        return result;
    }
    ...
    template<class BaseType, simd_index IndexType>
    requires(!simd_arithmetic<BaseType>)
    inline auto load_rvalue(auto&& base, IndexType const& idx)
    {
        decltype(simdized_value<index_model_t<IndexType>>(std::declval<BaseType>())) result;
        for(decltype(idx.size()) i = 0, e = idx.size(); i < e; ++i)
        {
            simd_members([&](auto&& dest, auto&& src) { dest[i] = src; }, result, base[scalar_index(idx, i)]);
        }
        return result;
    }
    ...
    requires(!simd_arithmetic<T>)
    inline void store(linear_location<T, SimdModel> const& location, ExprType const& expr)
    {
        const decltype(simdized_value<SimdModel>(std::declval<T>()))& source = expr;
        simd_members(
            [&](auto&& dest, auto&& src)
            { store<ElementSize>(linear_location<std::remove_reference_t<decltype(dest)>, SimdModel>{&dest}, src); },
            *location.base_,
            source);
    }
    ...
    requires(!simd_arithmetic<T>)
    inline auto load(indexed_location<T, SimdModel, IndexArray> const& location)
    {
        auto result = simdized_value<SimdModel>(*location.base_);
        simd_members(
            [&](auto&& dest, auto&& src)
            {
                dest = load<ElementSize>(
                    indexed_location<std::remove_reference_t<decltype(src)>, SimdModel, IndexArray>{
                        &src,
                        location.indices_});
            },
            result,
    ...
    requires(!simd_arithmetic<T>)
    inline void store(indexed_location<T, SimdModel, IndexArray> const& location, ExprType const& expr)
    {
        const decltype(simdized_value<SimdModel>(std::declval<T>()))& source = expr;
        simd_members(
            [&](auto&& dest, auto&& src)
    ...
    inline auto where(M const& mask, T& dest)
    {
    ...
    struct where_expression
    {
         ...
         */
        where_expression(M const& m, T& dest) : mask_(m), destination_(dest)
        {
        }

        /// Assignment operator. Only those vector lanes elements are assigned to destination, which are set in the
        /// mask.
        /**
...
namespace simd_access
{
    ...
    /**
     * Explicitly casts a value to its simdized value depending on the type of the simd index. A conversion from a
     * scalar value to its simdized value must exist. This can be used to resolve dependent contexts.
     * @tparam IndexType Explicitly given type of the index.
     * @param value Value to be casted.
     * @return If IndexType is a simd index, then the simdized version of value (usually with its inner values
     * broadcasted). Otherwise `value`.
     */
...
namespace simd_access
{
     ...
     */
    inline auto element(simd_index auto const& index, int i)
    {
     ...
     */
    template<int I>
    inline auto const& get_element(auto const& x)
    {
        static_assert(I == 0);
        return x;
    }
    ...
    inline decltype(auto) get_element(ValueType const& x)
    {
...
namespace simd_access
{
    ...
    inline void store(linear_location<T, SimdModel> const& location, stdx::simd<T, Abi> const& source)
    {
    ...
    inline void store(indexed_location<T, SimdModel, ArrayType> const& location, stdx::simd<T, Abi> const& source)
    {
        // scatter with indirect indices
        for(int i = 0; i < source.size(); ++i)
        {
            *reinterpret_cast<T*>(reinterpret_cast<char*>(location.base_) + ElementSize * location.indices_[i])
                = source[i];
        }
    }
            ...
            return ResultType(
                [&](int i)
                {
                    return *reinterpret_cast<T const*>(
                        reinterpret_cast<char const*>(location.base_) + ElementSize * i);
                });
        }
    }

    /**
     * Loads a simd value from a memory location defined by a base address and an indirect index. The simd elements to
     * be loaded are stored at the positions base+indices[0]*ElementSize, base+indices[1]*ElementSize, ...
     * @tparam ElementSize Size in bytes of the type of the simd-indexed element.
     * @tparam T Deduced type of a simd element.
     * @tparam SimdModel Simd type acting as type model.
     * @tparam ArrayType Deduced type of the array storing the indices.
    ...
    inline auto load(indexed_location<T, SimdModel, ArrayType> const& location)
    {
        using ResultType = stdx::rebind_simd_t<std::remove_const_t<T>, SimdModel>;
        ...
        return ResultType(
            ...
            {
                return *reinterpret_cast<T const*>(
                    reinterpret_cast<char const*>(location.base_) + ElementSize * location.indices_[i]);
            });
    }
    ...
    {
        using ResultType = stdx::rebind_simd_t<BaseType, index_model_t<IndexType>>;
        return ResultType([&](auto i) { return subobject(base[scalar_index(idx, i)]); });
    }
...
namespace simd_access
{
     ...
     * @param fn Generic function to be called. Takes one argument, whose type is either `index<SimdSize,
     * IntegralType>` or `IntegralType`.
     * @param residualLoopPolicy Determines the execution policy of residual iterations. If `ScalarResidualLoop`,
     * residual iterations are executed one by one. If `VectorResidualLoop`, residual iterations are executed
     * vectorized. In that case the user is responsible for the handling of indices possbily extending the valid
     * iteration range. Defaults to `ScalarResidualLoop`.
     */
    template<class SimdModel, auto... Args, typename ResidualLoopPolicyType = ScalarResidualLoopT>
    inline void loop(
        std::integral auto start,
        ...
        ResidualLoopPolicyType residualLoopPolicy = ScalarResidualLoop)
    {
        ...
        auto const simdSize = simd_i.size();
        auto const endOffset = residualLoopPolicy == ScalarResidualLoop ? 1 : simdSize;
     ...
     * `alignTestFn` returns true for a specific index. From there on `alignTestFn` isn't called anymore and the
     * function is called with a simd index. The remainder loop is called with an integral index again.
     * @tparam SimdModel Simd type acting as type model.
     * @tparam Args Optional additional template arguments passed to the function call operator.
     * @param start Start of the iteration range [start, end).
     * @param end End of the iteration range [start, end).
     * @param alignTestFn Generic function to be called. Takes one scalar argument of the common type of `start` and
     * `end`. Once it returns true, it isn't called anymore and the function starts to call `fn` with simd indices
    ...
    template<
        class SimdModel,
        auto... Args,
        std::random_access_iterator IteratorType,
        ...
        ResidualLoopPolicyType residualLoopPolicy = ScalarResidualLoop)
    {
        ...
        auto const simdSize = SimdIndexType::size();
        auto const endOffset = residualLoopPolicy == ScalarResidualLoop ? 1 : simdSize;
    ...
    template<
        ...
        std::random_access_iterator IteratorType,
        typename ResidualLoopPolicyType = ScalarResidualLoopT>
    inline void loop_with_linear_index(
        IteratorType start,
        IteratorType const& end,
        auto&& fn,
        ResidualLoopPolicyType residualLoopPolicy = ScalarResidualLoop)
...
namespace simd_access
{
    ...
    struct universal_simd : std::array<T, SimdModel::size()>
    {
        ...
        explicit universal_simd(G&& generator)
            : std::array<T, size()>(
                  [&]<int... I>(std::integer_sequence<int, I...>) -> std::array<T, size()>
                  {
    ...
    inline decltype(auto) generate_universal(IndexType const& idx, auto&& generator)
    {
        return universal_simd<decltype(generator(scalar_index(idx, 0))), index_model_t<IndexType>>(
            [&](auto i) { return generator(scalar_index(idx, i)); });
    }
    ...
    inline auto universal_access(simd_access::universal_simd<T, SimdModel> const& v, Func&& subobject)
    {
        using ScalarType = decltype(subobject(static_cast<std::unwrap_reference_t<T> const&>(v[0])));
        ...
        for(size_t i = 0; i < SimdModel::size(); ++i)
        {
            simd_members(
                [&](auto&& d, auto&& s) { d[i] = s; },
                result,
                subobject(static_cast<std::unwrap_reference_t<T> const&>(v[i])));
        }
        return result;
    }
 ...
 * @param expr A token sequence, which forms a valid `v expr` sequence for a scalar `v`. If `value` is a universal
 * simd, then `v` is an entry, otherwise `v` is `value`.
 */
...
namespace simd_access
{
    ...
    /**
     * This CRTP base class is needed, if the member operator is overloaded and the types, whose members are
     * potentially accessed might not be classes.
    ...
    struct member_overload<T, Derived>
    {
        ...
        /**
         * An overload of `operator.()` for data members must have exactly the form below. The type of the object,
         * whose member is accessed, must be specified in the template argument to enable the compiler to look up for
    ...
    struct cast_overload<T, Derived>
    {
        ...
        operator auto() const
        {
            return (static_cast<Derived const*>(this))->to_simd();
        }
    };
...
#    define VALUE_ACCESS_BIN_OP(op)                                                                                   \
        auto operator op(const auto& source)                                                                          \
        {                                                                                                             \
            return to_simd() op source;                                                                               \
        }
...
#    define VALUE_ACCESS_BIN_ASSIGNMENT_OP(op)                                                                        \
        void operator op##=(const auto& source) &&                                                                    \
        {                                                                                                             \
            store<ElementSize>(location_, to_simd() op source);                                                       \
        }
...
#    define VALUE_ACCESS_SCALAR_BIN_OP(op)                                                                            \
    ...
    public:
         ...
         */
        void operator=(auto const& source) &&
        {
         ...
         */
        value_access(Location const& location) : location_(location)
        {
        }
    ...
    inline auto make_value_access(Location const& location)
    {
    ...
    template<class PotentialSimdType>
    concept has_to_simd = requires(PotentialSimdType x) { x.to_simd(); };
...
namespace simd_access
{
    ...
    struct LValueSeparator<true>
    {
        ...
        static auto get_base_address(auto&& base_addr, index<SimdModel, IndexType> const& i)
        {
        ...
        static auto get_base_address(auto&& base_addr, stdx::simd<IndexType, Abi> const&)
        {
            return &base_addr[0];
        }
         ...
         */
        template<class SimdModel, std::integral IndexType>
        static auto get_base_address(auto&& base_addr, index<SimdModel, IndexType> const& i, auto&& subobject)
        {
            return &subobject(base_addr[i.index_]);
        }

        /// Computes the base address of a member of array elements for an indirect simd access using indices in
        /// `stdx::simd`.
        /**
         * @tparam IndexType Deduced integral type of the scalar index.
         * @tparam Abi Deduced abi of the `simd` paramter.
         ...
         */
        template<std::integral IndexType, class Abi>
        static auto get_base_address(auto&& base_addr, stdx::simd<IndexType, Abi> const&, auto&& subobject)
        {
        ...
        static auto get_direct_value_access(T* base, index<SimdModel, IndexType> const&)
        {
        ...
        static auto get_direct_value_access(T* base, stdx::simd<IndexType, Abi> const& idx)
        {
        ...
        requires(!std::integral<IndexType>)
        static auto to_simd(auto&& base, IndexType const& indices, Func&&... subobject)
        {
            return get_direct_value_access<sizeof(decltype(base[0]))>(
                get_base_address(base, indices, subobject...),
                indices);
        }
    };
    ...
    struct LValueSeparator<false>
    {
        ...
        template<class T, class IndexType>
        requires(!std::integral<IndexType>)
        static auto to_simd(T&& base, IndexType const& idx)
        {
        ...
        requires(!std::integral<IndexType>)
        static auto to_simd(T&& base, IndexType const& idx, Func&& subelement)
        {
     ...
     * @tparam IndexType Deduced type of the index to the base array. Can be a simd index or an ordinary integral
     * index.
     ...
     */
    template<class T, class IndexType>
    inline decltype(auto) sa(T&& base, IndexType const& index)
    {
    ...
    inline auto to_simd(T const& value)
    {
...
#define SIMD_ACCESS(base, index, ...)                                                                                 \
    simd_access::LValueSeparator<std::is_lvalue_reference_v<decltype((base[0] __VA_ARGS__))>>::to_simd(               \
        base,                                                                                                         \
        index __VA_OPT__(, [&](auto&& e) -> decltype((e __VA_ARGS__)) { return e __VA_ARGS__; }))
