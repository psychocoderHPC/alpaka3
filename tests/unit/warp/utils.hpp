...
namespace alpaka::test::warp
{
    ...
    ALPAKA_FN_HOST_ACC constexpr std::uint64_t fullMask(std::uint32_t warpSize)
    {
        if(warpSize == 0u)
        {
            return 0u;
    ...
    ALPAKA_FN_HOST_ACC constexpr std::uint64_t evenMask(std::uint32_t warpSize)
    {
    ...
    ALPAKA_FN_HOST_ACC constexpr std::uint64_t singleBit(std::uint32_t lane)
    {
        if(lane >= 64u)
        {
            return 0u;
